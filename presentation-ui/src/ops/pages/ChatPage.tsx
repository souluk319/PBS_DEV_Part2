...
type ChatPageProps = {
  ...
  incomingHandoff?: HandoffPayload | null;
...
function OpsHandoffResumeCard({
  ...
  return (
    <div className="surface-muted rounded-xl border border-primary/20 bg-primary/10 px-4 py-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="space-y-1">
          ...
          <div className="font-medium text-foreground">
            {getHandoffBotLabel(handoff.fromBot)} summary is ready for Studio Ops.
          </div>
          <div className="text-sm leading-6 text-muted-foreground">
            {getHandoffReasonLabel(handoff.handoffReason)}
          </div>
          <div className="text-sm text-foreground">{handoff.intentSummary}</div>
          {isPrepared ? (
            <div className="text-xs text-muted-foreground">
              handoff summary가 composer에 준비되었습니다.
            </div>
          ) : null}
        </div>
        <div className="flex flex-wrap gap-2">
...
export function ChatPage({
  ...
  const [resumePreparedHandoffId, setResumePreparedHandoffId] = useState("");
  ...
  function handleResumeHandoff() {
    if (!incomingHandoff) {
      return;
    }
    setDraft((current) => current.trim() || incomingHandoff.intentSummary);
    setResumePreparedHandoffId(incomingHandoff.handoffId);
    updateSession(session.id, (current) => ({
      ...current,
      title:
        current.title === "New Session"
          ? deriveChatSessionTitle(incomingHandoff.intentSummary)
          : current.title,
    }));
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Studio Ops"
        title="Studio Ops inside PlayBook Studio"
        description="PlayBook Studio 안에서 문서 답변과 live OCP 결과를 함께 다루고, 리소스 목록과 YAML 편집, grounded 근거 확인까지 이어지는 운영 대화 작업 공간입니다."
      />
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start">
        ...
        >
          ...
          <CardContent className="flex min-h-[70vh] flex-col gap-4 xl:h-[calc(100vh-11rem)] xl:min-h-0">
            ...
            {incomingHandoff ? (
              <OpsHandoffResumeCard
                handoff={incomingHandoff}
                isPrepared={resumePreparedHandoffId === incomingHandoff.handoffId}
                onResume={handleResumeHandoff}
