...
현재 주문은 `PlayBook Studio + OCP Ops` 를 `하나의 enterprise service` 로 통합하는 것이며,
이 작업은 `기존 PBS architecture 안에서 수행` 해야 한다.

현재 기준선은 아래다.

- `Playbook` 은 메인 서사이자 기본 진입 branch
- `Studio Ops` 는 고급 운영 surface
- `Playbot` 과 `OCP Ops bot` 은 분리 공존
- `cross-handoff` 는 핵심 제품 기능
- `document truth = PBS`
- `operational truth = Ops`
- `PBS-only enhancement backlog` 는 통합 blocker 가 아니면 후순위

## Current Renewal Scope

현재 운영 기준선은 아래다.

- `integrated_service_execution stage`
- `shared landing retained with stronger header branching`
- `Playbook main surface + Studio Ops advanced surface`
- `customer order: PlayBook Studio for OpenShift style integrated delivery`
...
1. `service integration inside PBS architecture`
2. `shared shell, branching, and handoff execution`
3. `document truth / operational truth ownership lock`
4. `corpus 와 citation 정합성 고정`
5. `official source-first operator coverage`
6. `reader / runtime hardening when it unblocks integration`
7. `customer/private pack lane stability when ordered`
